...
class Route {
  ...
  getOption = (url, data) => {
    ...
    if (['http', 'https'].includes(type)) {
      ...
      // const [key, tabbbarKey = 'index'] = page.split('|')
      // const index = tabbar.keyRouteIndex[tabbbarKey]
      // if (typeof index !== 'number') {
      //   console.error(tabbbarKey + ' 对应的tabbar没有在页面中被注册，无法跳转')
      //   return false
      // }
      // // 跳转到对应的key
      // tabbar.change(key, tabbbarKey)
      // // 跳转到对应Tabbar所在页面
      // this.back(Taro.getCurrentPages().length - index)
      return false
    }
